...
def open_chapter(chapterpath, archive=None):
    ...
    def find_f_img_folder():
        filepath = os.path.join(temp_p, [x for x in sorted([y.name for y in scandir(temp_p)])\
    ...
    def find_f_img_archive(extract=True):
        ...
        if extract:
            ...
            if send_folder:
                ...
                filepath = os.path.join(t_p, [x for x in sorted([y.name for y in scandir(t_p)])\
                    if x.lower().endswith(IMG_FILES) and not x.startswith('.')][0]) # Find first page
...
def get_gallery_img(gallery_or_path, chap_number=0):
    ...
    if is_archive:
        ...
        first_img = sorted([img.name for img in scandir(real_path) if img.name.lower().endswith(tuple(IMG_FILES)) and not img.name.startswith('.')])
        if first_img:
...
def make_chapters(gallery_object):
    ...
    try:
        log_d('Listing dir...')
        con = scandir(path) # list all folders in gallery dir
        ...
        if len(chapters) != 0:
            ...
            for ch in chapters:
                ...
                chap.pages = len([x for x in scandir(chap.path) if x.name.lower().endswith(IMG_FILES)])
            ...
            chap.pages = len([x for x in scandir(path) if x.name.lower().endswith(IMG_FILES)])
